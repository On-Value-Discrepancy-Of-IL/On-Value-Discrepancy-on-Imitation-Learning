...
def build_env(args):
    ...
    if env_type in {'atari', 'retro'}:
        ...
        env = make_vec_env(env_id, env_type, args.num_env or 1, seed, reward_scale=args.reward_scale,
                           flatten_dict_observations=flatten_dict_observations, collect_sample=args.collect_sample)

        if env_type == 'mujoco':
            env = VecNormalize(env, ob=False, ret=False)
            logger.info("**************No Normalization For MuJoCo***************************")
...
def main(args, extra_args):
    ...
    if MPI is None or MPI.COMM_WORLD.Get_rank() == 0:
        ...
        for sub_path in os.listdir(os.getcwd()):
            if sub_path == 'logs' or 'git' in sub_path:
                continue
            if os.path.isdir(sub_path):
                shutil.copytree(os.path.join(os.getcwd(), sub_path), os.path.join(path, "code", sub_path))
    else:

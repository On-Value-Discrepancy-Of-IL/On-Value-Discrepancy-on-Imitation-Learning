...
def load_results(root_dir_or_dirs, enable_progress=True, enable_monitor=True, verbose=False):
    ...
    for rootdir in rootdirs:
        ...
        result_walk = list(os.walk(rootdir))
        result_walk.sort()
        for dirname, dirs, files in result_walk:
            if '-proc' in dirname:
                files[:] = []
                continue
...
def plot_results(
    ...
    # todo
    groups = ['novelty-pursuit', 'learning-progress', 'random-selection']
    # groups = ['reward shaping', 'HER', 'distance reward']
    # todo
    ...
    for (isplit, sk) in enumerate(sorted(sk2r.keys())):
        ...
        if average_group:
            for group in groups:
                ...
                # todo : reset after experiment
                color = {'novelty-pursuit': 'red',
                         'bonus': 'blue',
                         'vanilla': 'darkgreen',
                         'HER': 'orange',
                         'distance reward': 'darkgreen',
                         'reward shaping': 'red',
                         'random-selection': 'darkgreen'}.get(group)
                ...
                print('{} | mean:{:.2f}, std:{:.2f}, stderr:{:.2f}'.format(group,
                    np.mean(ymean[-10:]),
                    np.mean(ystd[-10:]),
                    np.mean(ystderr[-10:]),
                ))
                ...
                g2l[group] = l
                if shaded_err:
                    ax.fill_between(usex, ymean - ystderr, ymean + ystderr, color=color, alpha=.1)
                if shaded_std:
        ...
        if any(g2l.keys()):
            ax.legend(
                g2l.values(),
                # ['%s'%(g, g2c[g]) for g in g2l] if average_group else g2l.keys(),

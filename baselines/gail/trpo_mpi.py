...
def learn(env, policy_func, reward_giver, expert_dataset, rank,
    ...
    linear_reward = hasattr(reward_giver, 'simplex')
    ...
    if d_adam is not None: d_adam.sync()
    vfadam.sync()
    if rank == 0:
        logger.info('\n---------------------------GAIL--------------------------')
    ...
    while True:
        ...
        if linear_reward:
            ...
            if hasattr(reward_giver, "obs_rms"):
                reward_giver.obs_rms.update(np.concatenate([ob, ac], axis=1))
            # update w
            results = reward_giver.fit(ob, ac)
            gap = results['gap']
            logger.info('-------------Linear Reward----------------')
            ...
            logger.info(fmt_row(13, reward_giver.loss_name))
            logger.info(fmt_row(13, np.mean(d_losses, axis=0)))
        ...
        logger.record_tabular("fps", timesteps_so_far//(time.time() - tstart))
